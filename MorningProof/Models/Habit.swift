...
enum HabitType: String, Codable, CaseIterable, Identifiable {
    ...
    var tier: HabitVerificationTier {
        ...
        case .coldShower, .noSnooze, .morningStretch, .meditation, .prayer, .reading, .walkDog: return .honorSystem
        }
    }
    ...
    var category: HabitCategory {
        ...
        case .meditation, .coldShower, .sunlightExposure, .hydration, .prayer, .reading: return .wellness
        }
    }
...
struct HabitConfig: Codable, Identifiable {
    ...
    static var defaultConfigs: [HabitConfig] {
        HabitType.allCases.enumerated().map { index, type in
            HabitConfig(
                habitType: type,
                isEnabled: [.madeBed, .sleepDuration, .prayer, .morningSteps].contains(type),
                displayOrder: index
            )
        }
    }
