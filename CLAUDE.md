...
- **Adding AI-verified habits**: When adding a new AI-verified (camera-based) habit type, you MUST add it to the `specialInputHabits` set in `DashboardView.swift`'s `isHoldToCompleteHabit()` function. Any habit NOT in that set defaults to hold-to-complete UI. The tap handler (`handleHabitTap`) may already route correctly, but without adding to `specialInputHabits` the row will show the wrong UI.
